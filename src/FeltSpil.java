...
public class FeltSpil {
    ...
    public static void main(String[] args) {
        ...
        if (language==2){
            System.out.println("""
                Both players start with a cash balance of DKK 1,000.
                Roll the dice to land on a given field, with a given value,
                which can be both positive or negative.
                The player who first reaches a cash balance of DKK 3,000 wins
                """); // Text block (forslået af java)
        }
        ...
        if (language==2){
            System.out.println("Player 1, write thy name");
        }
        ...
        if (language==2){
            System.out.println("Player 2, write thy name");
        }
        ...
        if (language==1) {
            ...
            while (activePlayer.getBalance() < 3000) {
                ...
                System.out.print("Press enter to roll die: It is " + activePlayer.getName() + "'s turn");
                new Scanner(System.in).nextLine();

                faceTotal = terning1.roll() + terning2.roll();

                if (faceTotal < 2 || faceTotal > 12) {
                    System.out.println("Only fields 2-12 exist, you rolled " + faceTotal);
                } else {
                    Field field = fieldMap.get(faceTotal); // Ny variabel field som har typen Felt tilføjes
                    activePlayer.addScore(field.point);
                    // Tilføjer et givent felt's point til den aktive spilleres pengebeholdning
                    // vha. metoden addScore som er defineret i Player klassen
                    System.out.println("\n" + field.description);
                    System.out.println("you rolled " + field.point + " point.");
                }
                System.out.println(activePlayer.getName() + "'s new score is: " + activePlayer.getBalance());

                if (activePlayer == player1) {
                    activePlayer = player2; // Hvis activePlayer er player1, skift den aktive spiller til at være player2
                    if (player1.getBalance() >= 3000) {
                        System.out.println(player1.getName() + " won with a score of " + player1.getBalance() + " point.");
                        System.out.println(player2.getName() + " lost with a score of " + player2.getBalance() + " point.");
                        System.exit(0);
                    } else if (faceTotal == 10) {
                        System.out.println("since you landed on " + F10.name + " youre getting an extra turn");
                        activePlayer = player1;
                    }
                } else {
                    activePlayer = player1; // Hvis activePlayer er player2, skift den aktive spiller til at være player1
                    if (player2.getBalance() >= 3000) {
                        System.out.println(player2.getName() + " won with a score of " + player2.getBalance() + " point.");
                        System.out.println(player1.getName() + " lost with a score of " + player1.getBalance() + " point.");
                        System.exit( 1);
                    } else if (faceTotal == 10) {
                        System.out.println("since you landed on " + F10.name + " youre getting an extra turn");
                        activePlayer = player2;
